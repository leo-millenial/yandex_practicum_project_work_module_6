...
trait Parser {
    ...
    fn parse<'a>(&self, input: &'a str) -> ParseResult<'a, Self::Dest>;
}
...
mod stdp {
    ...
    impl Parser for U32 {
        ...
        fn parse<'a>(&self, input: &'a str) -> ParseResult<'a, Self::Dest> {
            ...
            let value = std::num::NonZeroU32::new(value).ok_or(())?.get();
...
fn do_unquote(input: &str) -> ParseResult<'_, String> {
...
fn do_unquote_non_escaped(input: &str) -> ParseResult<'_, &str> {
...
impl Parsable for AuthData {
    ...
    fn parser() -> Self::Parser {
        map(take(AUTHDATA_SIZE, stdp::Byte), |authdata| {
            let mut bytes = [0; AUTHDATA_SIZE];
            bytes.copy_from_slice(&authdata);
            AuthData(Box::new(bytes))
        })
    }
}
...
fn just_parse<T: Parsable>(input: &str) -> ParseResult<'_, T> {
...
pub fn parse_announcements(input: &str) -> ParseResult<'_, Announcements> {
    just_parse(input)
}

pub fn parse_log_line(input: &str) -> ParseResult<'_, LogLine> {
    just_parse(input)
}
